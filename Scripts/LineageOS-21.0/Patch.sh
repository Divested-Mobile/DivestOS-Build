...
if [ "$DOS_GRAPHENE_MALLOC" = true ]; then #(GrapheneOS)
...
if [ "$DOS_GRAPHENE_MALLOC" = true ]; then applyPatch "$DOS_PATCHES/android_build_soong/0002-hm_available.patch"; fi; #(GrapheneOS)
fi;
...
applyPatch "$DOS_PATCHES/android_frameworks_base/0018-Exec_Based_Spawning-4.patch"; #Pass through fullPreload to libcore (GwrapheneOS)
...
#applyPatch "$DOS_PATCHES/android_frameworks_base/0029-Strict_Package_Checks-3.patch"; #Require fs-verity when installing system package updates (GrapheneOS) #XXX: F-Droid doesn't support fs-verity metadata
#applyPatch "$DOS_PATCHES/android_frameworks_base/0029-Strict_Package_Checks-4.patch"; #Don't allow updating system packages to the same versionCode (GrapheneOS) #XXX: depends on previous patch
...
#if [ "$DOS_MICROG_SUPPORT" = true ]; then  #TODO: 21REBASE
